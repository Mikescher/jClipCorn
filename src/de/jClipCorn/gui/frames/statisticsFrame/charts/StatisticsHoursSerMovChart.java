...
public class StatisticsHoursSerMovChart extends StatisticsChart {

	public StatisticsHoursSerMovChart(CCMovieList ml) {
		super(ml);
	}
	...
	private XYDataset getDataSet(CCMovieList movielist) {
		...
		int[] allpos_ser = StatisticsHelper.getSeriesMinuteCountForAllDates(movielist, mindate, daycount);
		int[] allpos_mov = StatisticsHelper.getMovieMinuteCountForAllDates(movielist, mindate, daycount);
		
		double[][] series = new double[2][daycount];
		
		for (int i = 0; i < daycount; i++) {
			series[0][i] = minMilliecs + i * CCDate.MILLISECONDS_PER_DAY;
			series[1][i] = allpos_ser[i] + allpos_mov[i];
		}
	...
	protected String createTitle() {
		return LocaleBundle.getString("StatisticsFrame.charttitles.hoursSerMov"); //$NON-NLS-1$
	}
