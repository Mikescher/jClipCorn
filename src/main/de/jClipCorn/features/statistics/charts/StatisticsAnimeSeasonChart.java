...
public class StatisticsAnimeSeasonChart extends StatisticsChart {

	public StatisticsAnimeSeasonChart(CCMovieList ml, StatisticsTypeFilter _source) {
		super(ml, _source);
	}
	...
	protected JFreeChart createChart(CCMovieList movielist, StatisticsTypeFilter source) {
		...
		// Custom tooltip generator to show full season information
		renderer.setSeriesToolTipGenerator(0, (dataset, row, column) -> {
			String shortLabel = (String) dataset.getColumnKey(column);
		...
		plot.getDomainAxis().setCategoryLabelPositions(CategoryLabelPositions.createUpRotationLabelPositions(Math.PI/2));
	...
	private DefaultCategoryDataset getDataSet(CCMovieList movielist, StatisticsTypeFilter source) {
		...
		HashMap<String, Integer> seasonValues = StatisticsHelper.getCountForAllAnimeSeasons(it);
		
		// Sort anime seasons chronologically
		List<String> sortedSeasons = new ArrayList<>(seasonValues.keySet());
		sortedSeasons.sort(new CCAnimeSeasonComparator());

		for (String fullSeason : sortedSeasons) {
			Integer count = seasonValues.get(fullSeason);
			dataset.addValue(count, "Series0", fullSeason); //$NON-NLS-1$
		}
	...
	public String createTitle() {
		return LocaleBundle.getString("StatisticsFrame.charttitles.animeseason"); //$NON-NLS-1$
	}
