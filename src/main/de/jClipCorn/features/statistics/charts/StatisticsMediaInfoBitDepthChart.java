...
public class StatisticsMediaInfoBitDepthChart extends StatisticsChart {
	public StatisticsMediaInfoBitDepthChart(CCMovieList ml, StatisticsTypeFilter _source) {
		super(ml, _source);
	}
	...
	private DefaultCategoryDataset getDataSet(CCMovieList movielist, StatisticsTypeFilter source) {
		Map<Integer, Integer> values = source
				...
				.map(e -> e.mediaInfo().get().Bitdepth)
	...
	public String createTitle() {
		return LocaleBundle.getString("StatisticsFrame.charttitles.mediainfo_bitdepth"); //$NON-NLS-1$
	}
