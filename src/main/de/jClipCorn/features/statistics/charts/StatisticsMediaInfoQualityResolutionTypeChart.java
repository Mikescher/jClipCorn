...
public class StatisticsMediaInfoQualityResolutionTypeChart extends StatisticsChart {
	public StatisticsMediaInfoQualityResolutionTypeChart(CCMovieList ml, StatisticsTypeFilter _source) {
		super(ml, _source);
	}
	...
	private DefaultCategoryDataset getDataSet(CCMovieList movielist, StatisticsTypeFilter source) {
		...
		for (CCQualityResolutionType qc : CCQualityResolutionType.values()) {
			if (qc == CCQualityResolutionType.MULTIPLE) continue;
			if (qc == CCQualityResolutionType.OTHER) continue;
			int v = values.getOrDefault(qc, 0);
			if (v == 0) continue;
			dataset.addValue(v, "Series0", qc.Text); //$NON-NLS-1$
		}
	...
	public String createTitle() {
		return LocaleBundle.getString("StatisticsFrame.charttitles.mediainfo_qualitytext"); //$NON-NLS-1$
	}
