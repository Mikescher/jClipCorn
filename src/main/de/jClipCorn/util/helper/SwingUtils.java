...
public final class SwingUtils {
	...
	private SwingUtils() {
		throw new Error("SwingUtils is just a container for static methods"); //$NON-NLS-1$
	}

	/**
	 * Convenience method for searching below <code>container</code> in the component hierarchy and return nested components that are instances of class <code>clazz</code> it finds. Returns an empty list if no such components exist in the container.
	 * <P>
	...
	/**
	 * Convenience method that searches below <code>container</code> in the component hierarchy and returns the first found component that is an instance of class <code>clazz</code> having the bound property value. Returns {@code null} if such component cannot be found.
	 * <P>
	 ...
	 *            the container at which to begin the search
	 * @param property
	 *            the className of the bound property, exactly as expressed in the accessor e.g. "Text" for getText(), "Value" for getValue().
	 * @param value
	 ...
	 *            the value of the bound property
	 * @param nested
	 *            true to list components nested within another component which is also an instance of <code>clazz</code>, false otherwise
	 * @return the component, or null if no such component exists in the container
	 * @throws java.lang.IllegalArgumentException
	 *             if the bound property does not exist for the class or cannot be accessed
	 */
	...
	/**
	 * Convenience method that searches below <code>container</code> in the component hierarchy in a depth first manner and returns the first found component of class <code>clazz</code> having the bound property value.
	 * <P>
	 ...
	 *            the container at which to begin the search
	 * @param property
	 *            the className of the bound property, exactly as expressed in the accessor e.g. "Text" for getText(), "Value" for getValue(). This parameter is case sensitive.
	 * @param value
	 *            the value of the bound property
	...
	private static <T extends JComponent> T getComponentFromList(Class<T> clazz, List<T> list, String property, Object value) throws IllegalArgumentException {
		...
		Method method = null;
		try {
			method = clazz.getMethod("get" + property); //$NON-NLS-1$
		} catch (NoSuchMethodException ex) {
			try {
				method = clazz.getMethod("is" + property); //$NON-NLS-1$
			} catch (NoSuchMethodException ex1) {
				throw new IllegalArgumentException("Property " + property + //$NON-NLS-1$
						" not found in class " + clazz.getName()); //$NON-NLS-1$
			}
		}
		try {
			for (T t : list) {
			...
			throw new IllegalArgumentException("Error accessing property " + property + //$NON-NLS-1$
					" in class " + clazz.getName()); //$NON-NLS-1$
		} catch (IllegalAccessException ex) {
			throw new IllegalArgumentException("Property " + property + //$NON-NLS-1$
					" cannot be accessed in class " + clazz.getName()); //$NON-NLS-1$
	...
	static {
		...
		setExclude.add("getColorModel"); //$NON-NLS-1$
		setExclude.add("getGraphics"); //$NON-NLS-1$
		setExclude.add("getGraphicsConfiguration"); //$NON-NLS-1$
	}
	...
	public static <T extends JComponent> Class<?> getJClass(T component) {
		Class<?> clazz = component.getClass();
		while (!clazz.getName().matches("javax.swing.J[^.]*$")) { //$NON-NLS-1$
